...
class Graph {
  ...
  // 너비 우선 탐색
  // 깊이 우선 탐색의 반복형 구현을 재활용
  // stack 대신 queue를 이용하여 구현
  // 1. 방문할 노드들을 차례로 저장할 queue 배열 함께 생성
  // 2. queue에서 shift() 함수로 노드를 차례로 꺼내와 visited 처리 및 result에 저장
  // 3. 해당 노드의 인접리스트를 순회
  // 4. stack 배열에 저장
